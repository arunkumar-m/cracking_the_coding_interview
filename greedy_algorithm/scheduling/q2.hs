...
instance Show Job where
  show (Job x y) = "[Job] " ++ "W: " ++ (show x) ++ " L: " ++ (show y) ++ " D: " ++ (show $ (fromIntegral x) / (fromIntegral y))
instance Eq Job where
  (Job x1 y1) == (Job x2 y2) = (x1 == x2) && (y1 == y2)
instance Ord Job where
  (Job x1 y1) <= (Job x2 y2) = (fromIntegral x1 / fromIntegral y1) < (fromIntegral x2 / fromIntegral y2)
